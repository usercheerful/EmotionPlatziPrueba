...
namespace EmotionPlatzi.Web.Controllers
{
    ...
    {
        //Accedemos a todo el conjunto de datos
        private EmotionPlatziWebContext db = new EmotionPlatziWebContext();

        // GET: EmoPictures
        public ActionResult Index() //el metodo es mismo nombre de la Vista
        {
            return View(db.EmoPictures.ToList()); //Como modelo enviamos un listado de datos
        }
        ...
        //SE EJECUTA ESTE METODO CUANDO PRESIONAMOS EL LINK PARA EDITAR ALGUN REGISTRO
        // GET: EmoPictures/Edit/5
        public ActionResult Edit(int? id)
        {
            //VERIFICAMOS SI NOS PASARON UN PARAMETRO
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            }
            //REALIZAMOS UNA BUSQUEDA DE DATOS POR EL POR EL PARAMETRO PASADO
            EmoPicture emoPicture = db.EmoPictures.Find(id);
            //VERIFICAMOS SI ENCONTRO ALGUN DATO
            if (emoPicture == null)
            {
                return HttpNotFound();
            }
            //ENVIAMOS EL OBJETO QUE SE ENCONTRO
            return View(emoPicture);
        }

        //SE EJECUTA ESTE METODO PARA GUARDAR LOS DATOS QUE HAN SIDO MODIFICADOS
